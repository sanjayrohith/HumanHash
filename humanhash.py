...
def print_hoodie():
    W = "\033[0m"
    C = "\033[1;36m" # Cyan/Blue
    
    print(f"{W}         .---.        ")
    print(f"{W}        /     \\       {C}# humanhash.py{W}")
    print(f"{W}       |  _ _  |      {C}# Social Engineering{W}")
    print(f"{W}       |   |   |      {C}# Dictionary Gen{W}")
    print(f"{W}      /`   _   `\\     ")
    print(f"{W}     /           \\    ")
    print(f"{W}    |   _     _   |   ")
    print("")
...
def version():
    ...
    print("\r\n	\033[1;31m[ humanhash.py ]  " + __version__ + "\033[1;m\r\n")
...
def improve_dictionary(file_to_open):
    ...
    if leetmode == "y":
        for (
            ...
            unique_lista
        ):  # if you want to add more leet chars, you will need to add more lines in humanhash.cfg too...
    ...
    print_to_file(file_to_open + ".humanhash.txt", unique_list_finished)
...
# the main function
def main():
    """Command-line interface to the humanhash utility"""

    read_config(os.path.join(os.path.dirname(os.path.realpath(__file__)), "humanhash.cfg"))
